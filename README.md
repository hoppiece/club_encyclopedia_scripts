...
変数`cols`の値は，返信メールに載せたい要素までの回答スプレッドシートの列数と同じにする．従来バージョンでは，回答スプレッドシートの列数を取得して`cols`に代入していたが，回答スプレッドシートで各顧客についての情報を追加したときのために，確認文の要素がこちらの好きな場所で打ち止めになるようにした．
